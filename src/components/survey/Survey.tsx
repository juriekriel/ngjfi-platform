...
export default function Survey({
  ...
  const [status, setStatus] = useState<"loading" | "ready" | "not_fielding" | "no_org">("loading");
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: o } = await sb
        .from("organisations")
        .select("id,slug,name,brand_color,country,welcome_message")
        .eq("short_name", slug)
        .maybeSingle();
      if (!o) {
        setStatus("no_org");
      ...
      const { data: c } = await sb
        ...
        .eq("slug", CAMPAIGN_SLUG[audience])
        .eq("active", true)
        .maybeSingle();
