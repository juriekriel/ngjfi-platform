...
export default function IntelligenceView({
  ...
  /**
   * Which data space this surface reads. "live" publishes only real
   * organisations and stays honestly empty until the critical-mass gate is
   * passed; "demo" aggregates the sandbox's fiction only within itself. The
   * split is enforced in the database, not here — this prop only chooses which
   * enforced function to call.
   */
  space?: "live" | "demo";
}) {
        ...
        <div className="mt-8 border-2 border-ink p-6">
          ...
          <p className="mt-4 max-w-2xl leading-relaxed text-slate">
            {d.reason === "below_critical_mass" ? (
              <>
                <b>{(d.completions ?? 0).toLocaleString()}</b> completions so far. A geography is
                only ever named once{" "}
                <b>{d.gate ? d.gate.toLocaleString() : "the agreed threshold of"}</b> people there
                have completed the Index.
              </>
            ) : (
              <>
                The first field wave has not been released yet. Every figure currently on this site
